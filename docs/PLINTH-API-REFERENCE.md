# Plinth API Reference

The complete contract for building widgets that run on the Plinth dashboard.
...
- `widget-api.js` and the iCUE shim are **injected automatically** into every widget; the
  `<script src="https://app.plinth/widget-api.js">` include is optional (kept for
...
`.plinthwidget` file. Install via tray → Install widget, the Settings window, or by dropping
the folder into `%LocalAppData%\Plinth\widgets\`. Files hot-reload.
...
WW.listMedia()       // -> Promise<[{name, url, kind: 'image'|'video'}]>; served from https://media.plinth/
...
`init.insecureMedia: true` (alongside `insecure`) additionally registers the host:port
for browser-layer certificate-error acceptance so a media element can stream from the
same self-signed server; bare `insecure` never does this — one widget's health probe
must not relax certificate handling for another widget's credentialed traffic.
...
| `ww-fetch` | `id, url, method, body, contentType, headers?, insecure?, insecureMedia?` | host-proxied fetch (CORS/bot-wall relief; GET/POST/PUT/HEAD; `insecure` honored only for private-IP hosts; `insecureMedia` also registers the host:port for browser-layer certificate-error acceptance) |
...
- Image/video files chosen in Settings are copied into
  `%LocalAppData%\Plinth\backgrounds\` under a content-hashed name and served to
  the shell from the `https://backgrounds.plinth/<file>` virtual host. `source` is just the
  file name.
