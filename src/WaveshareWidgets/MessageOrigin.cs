...
///
///   1. every widget is mapped to its own virtual host (`{slug}.widgets.wsw`), so a frame
///      is cross-origin with the shell on `app.wsw` and cannot reach `parent.chrome.webview`;
